...
public class NewsAnalyzerComunio {
	 ...
	 * @param lastUpdates           letzte Aktualisierungszeitstempel (wird ggf. aktualisiert)
	    ...
	    try {
	            ...
	            for (String dateKey : groups.keySet()) {
	                ...
	                for (int i = 0; i < entries.length(); i++) {
	                    ...
	                    if ("TRANSACTION_TRANSFER".equals(type)) {
	                        if (matchdayInfo != null && matchdayInfo.isSeasonOver(lastUpdates)) {
	                        ...
	                        for (String transferType : new String[]{"FROM_COMPUTER", "TO_COMPUTER", "BETWEEN_USERS"}) {
	                            ...
	                            for (int j = 0; j < transfers.length(); j++) {
	                                try {
	                                    ...
	                                    transferLogs.add(newsDate + " - TRANSFER: " + transfer.getJSONObject("tradable").getString("name")
	                                            + " von " + seller + " zu " + buyer);
	        ...
	        for (TransferNewsEntry entry : transferNewsList) {
	            ...
	            if (!Objects.equals(oldOwner, entry.getBuyerId())) {
	                ...
	                LOGGER.info("Mapping aktualisiert: Spieler " + entry.getPlayerId() + " -> User " + entry.getBuyerId() + " am " + entry.getDate());
	            }
