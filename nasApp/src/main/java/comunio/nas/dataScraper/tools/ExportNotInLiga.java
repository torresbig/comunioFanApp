...
/**
 * Dienstklasse für das Auffinden, Exportieren und Entfernen von Spielern, die nicht mehr in der Liga sind.
 */
public class ExportNotInLiga {
     ...
     */
    private static Map<String, JSONObject> findPlayerNichtInLiga(JSONArray playerDB) {
        ...
        for (int i = 0; i < playerDB.length(); i++) {
            ...
            if (id == null || id.isEmpty()) {
                LOGGER.warning("Spieler ohne gültige ID an Index " + i + ": " + playerObj);
                continue;
            }
            String verein = data.optString("verein");
            JSONObject status = data.optJSONObject("status");
            if (status == null) {
                LOGGER.warning("Kein 'status' Feld für Spieler " + id);
                continue;
            }
     ...
     */
    public static void exportAndRemoveNotInLiga(JSONObject playerDBObj, JSONObject notInligaDBObj, LastUpdates lastUpdates) {
        ...
        Map<String, JSONObject> notInLigaPlayerMap = findPlayerNichtInLiga(playerDB);
